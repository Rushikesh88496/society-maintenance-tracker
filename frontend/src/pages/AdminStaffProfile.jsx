...
export default function AdminStaffProfile() {
    ...
    return (
      <Layout>
        <div className="max-w-4xl mx-auto">
          <button onClick={() => navigate('/staff')} className="btn-ghost mb-6">
            <ChevronLeft size={16} /> Back to Staff
          </button>
          <div className="card p-12 text-center">
            <AlertCircle size={40} className="mx-auto text-surface-300 mb-4" />
            <h2 className="text-lg font-semibold text-surface-900">Staff Member Not Found</h2>
            <p className="text-sm text-surface-500 mt-1">This staff member does not exist or has been removed.</p>
          </div>
        </div>
      </Layout>
    );
  ...
  const stats = member.complaintStats || {};
  const roleColor = ROLE_COLORS[member.role] || 'bg-surface-100 text-surface-600';

  return (
    <Layout>
      <div className="max-w-4xl mx-auto space-y-6">
        <button onClick={() => navigate('/staff')} className="btn-ghost -ml-2">
          <ChevronLeft size={16} /> Back to Staff
        </button>
        ...
        <div className="card p-6 sm:p-8">
          <div className="flex flex-col sm:flex-row sm:items-start gap-6">
            <div className={`w-20 h-20 rounded-2xl flex items-center justify-center text-2xl font-bold flex-shrink-0 ${roleColor}`}>
              {initials}
            </div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                <div>
                  <div className="flex items-center gap-3 flex-wrap">
                    <h1 className="text-display-sm text-surface-900">{member.name}</h1>
                ...
                <button
                  ...
                  className={member.is_active ? 'btn-secondary text-danger border-danger-100 hover:bg-danger-50' : 'btn-success'}
                >
                  {member.is_active ? (
                    <><Ban size={16} /> Disable</>
                  ) : (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-6 pt-6 border-t border-surface-100">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium text-surface-900">{member.email || '---'}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-lg bg-surface-100 flex items-center justify-center flex-shrink-0">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium text-surface-900">{member.phone || '---'}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-lg bg-surface-100 flex items-center justify-center flex-shrink-0">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium text-surface-900">{member.role}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-lg bg-surface-100 flex items-center justify-center flex-shrink-0">
                  ...
                  <div>
                    <p className="text-xs font-medium text-surface-500">Added</p>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          <div className="card-hover p-4">
            <p className="text-xs font-medium text-surface-500">Total Assigned</p>
        ...
        <div className="card">
          <div className="p-6 pb-0">
            <h2 className="text-display-sm text-surface-900">Assigned Complaints</h2>
          </div>

          {member.complaints && member.complaints.length > 0 ? (
            ...
            <div className="p-12 text-center">
              ...
              <p className="text-sm text-surface-500">No complaints assigned to this staff member yet.</p>
            </div>
          )}
        </div>
      </div>
